...
export default function LessonScreen() {
  ...
  const { completeLesson, state } = useLearning();
  ...
  if (!lesson) return <SafeAreaView style={styles.center}><Text style={styles.title}>Không tìm thấy bài học</Text><ActionButton label="Về trang học" onPress={() => router.replace('/(tabs)')} /></SafeAreaView>;

  const previewImage = lesson.activities.find((item) => item.image)?.image;
  ...
  if (!started) return <SafeAreaView style={styles.safe}>
    ...
    <View style={styles.intro}>
      {previewImage && <Image source={previewImage} style={styles.introImage} resizeMode="cover" />}
      <View style={[styles.introIcon, { backgroundColor: lesson.color }, Boolean(previewImage) && styles.introIconOverlap]}><MaterialCommunityIcons name={lesson.icon as never} size={46} color="#FFFFFF" /></View>
      <Text style={styles.eyebrow}>BÀI HỌC MỚI</Text><Text style={styles.introTitle}>{lesson.title}</Text><Text style={styles.objective}>{lesson.objective}</Text>
      <View style={styles.metaRow}><Meta icon="clock-outline" value={`${lesson.estimatedMinutes} phút`} /><Meta icon="cards-outline" value={`${lesson.activities.length} hoạt động`} /><Meta icon="star-four-points" value={`${lesson.activities.reduce((sum, item) => sum + item.xp, 0)} XP`} /></View>
    </View>
    <View style={styles.bottom}><ActionButton label="Bắt đầu bài học" icon="play" color={lesson.color} onPress={() => setStarted(true)} /></View>
  </SafeAreaView>;
  ...
  const onAnswer = (isCorrect: boolean) => {
    ...
    if (isCorrect) {
      ...
      setEarnedXp((value) => value + activity.xp);
  ...
  const continueLesson = async () => {
    ...
    const xpEarned = Math.max(2, earnedXp);
  ...
  return <SafeAreaView style={styles.safe}>
    <View style={styles.playerHeader}>
      ...
      <View style={styles.heart}><MaterialCommunityIcons name="heart" size={22} color={Theme.colors.coral} /><Text style={styles.heartText}>{state.hearts}</Text></View>
    </View>
    <ScrollView contentContainerStyle={styles.playerContent} keyboardShouldPersistTaps="handled">
      <Text style={styles.counter}>HOẠT ĐỘNG {index + 1}/{lesson.activities.length}</Text>
...
const styles = StyleSheet.create({
  ...
  intro: { flex: 1, alignItems: 'center', justifyContent: 'center', padding: 26 }, introImage: { width: '100%', maxWidth: 460, aspectRatio: 16 / 9, borderRadius: 8, backgroundColor: '#E8EEF2' }, introIcon: { width: 104, height: 104, borderRadius: 52, alignItems: 'center', justifyContent: 'center', borderWidth: 7, borderColor: 'rgba(255,255,255,0.85)', ...Theme.shadow }, introIconOverlap: { marginTop: -52 },
