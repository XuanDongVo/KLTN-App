...
export default function ReviewScreen() {
  ...
  return <SafeAreaView style={styles.safe} edges={['top']}>
    <View style={styles.header}><View><Text style={styles.eyebrow}>SMART REVIEW</Text><Text style={styles.title}>On lai de nho lau</Text></View><View style={styles.count}><MaterialCommunityIcons name="brain" size={22} color={Theme.colors.violet} /><Text style={styles.countText}>{mistakes.length}</Text></View></View>
    <ScrollView contentContainerStyle={styles.content}>
      {mistakes.length === 0 ? <View style={styles.empty}><View style={styles.emptyIcon}><MaterialCommunityIcons name="check-decagram" size={58} color={Theme.colors.greenDark} /></View><Text style={styles.emptyTitle}>Hom nay da on xong!</Text><Text style={styles.emptyText}>Cac cau tra loi sai trong bai hoc se xuat hien tai day de be luyen lai.</Text></View> : mistakes.map((activity) => activity && <Pressable key={activity.id} onPress={() => setRevealed(revealed === activity.id ? undefined : activity.id)} style={styles.reviewCard}>
