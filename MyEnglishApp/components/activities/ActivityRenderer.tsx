...
export function ActivityRenderer({ activity, onAnswer }: Props) {
  ...
  if (activity.type === 'WORD_CARD') return <View style={styles.wordCard}>
    {activity.imageUrl && <Image source={{ uri: activity.imageUrl }} style={styles.heroImage} />}
    ...
    <ActionButton label="Da hieu" icon="check" onPress={() => onAnswer(true, 'learned')} />
  </View>;

  if (activity.type === 'IMAGE_CAPTION') return <PhotoMissionActivity onComplete={() => onAnswer(true, 'caption-created')} />;

  if (activity.type === 'SPEAK_REPEAT') return <View style={styles.centered}>
    <View style={styles.speakIcon}><MaterialCommunityIcons name="microphone" size={42} color={Theme.colors.coral} /></View>
    <Text style={styles.bigPrompt}>{activity.prompt}</Text><Text style={styles.helper}>{activity.instruction}</Text>
    <ActionButton label="Da doc xong" icon="microphone-check" color={Theme.colors.coral} onPress={() => onAnswer(true, 'spoken')} />
  </View>;

  if (activity.type === 'FILL_IN_BLANK') return <View style={styles.stack}>
    <Text style={styles.bigPrompt}>{activity.sentence}</Text>
    <TextInput value={input} onChangeText={setInput} autoCapitalize="none" autoCorrect={false} placeholder="Nhap tu con thieu" placeholderTextColor="#9AA8B1" style={styles.input} />
    <ActionButton label="Kiem tra" disabled={!input.trim()} onPress={checkInput} />
  </View>;

  if (activity.type === 'SENTENCE_BUILDER') return <View style={styles.stack}>
    <View style={styles.sentenceArea}>{builtWords.length ? builtWords.map((word, index) => <Pressable key={`${word}-${index}`} onPress={() => setBuiltWords((items) => items.filter((_, i) => i !== index))} style={styles.wordChipSelected}><Text style={styles.wordChipText}>{word}</Text></Pressable>) : <Text style={styles.placeholderSentence}>Cham vao cac tu theo dung thu tu</Text>}</View>
    ...
    <ActionButton label="Kiem tra" disabled={!builtWords.length} onPress={checkSentence} />
  </View>;

  if (activity.type === 'MATCHING') return <View style={styles.stack}>
    <Text style={styles.helper}>Chon mot tu ben trai, sau do chon nghia dung.</Text>
  ...
  if (activity.type === 'TRUE_FALSE') return <View style={styles.stack}>
    {activity.imageUrl && <Image source={{ uri: activity.imageUrl }} style={styles.heroImage} />}
    <View style={styles.choicesRow}>{[{ id: 'true', label: 'Dung', icon: 'check-circle' }, { id: 'false', label: 'Sai', icon: 'close-circle' }].map((choice) => <Pressable key={choice.id} onPress={() => setSelected(choice.id)} style={[styles.binaryChoice, selected === choice.id && styles.choiceSelected]}><MaterialCommunityIcons name={choice.icon as never} size={32} color={choice.id === 'true' ? Theme.colors.greenDark : Theme.colors.coralDark} /><Text style={styles.choiceLabel}>{choice.label}</Text></Pressable>)}</View>
    <ActionButton label="Kiem tra" disabled={!selected} onPress={() => onAnswer((selected === 'true') === activity.answer, selected)} />
  </View>;

  if (activity.imageUrl) return <View style={styles.stack}>
    ...
    <ActionButton label="Kiem tra" disabled={!selected} onPress={checkSelection} />
  </View>;

  return <View style={styles.stack}><ChoiceArea activity={activity} selected={selected} onSelect={setSelected} /><ActionButton label="Kiem tra" disabled={!selected} onPress={checkSelection} /></View>;
}

function ChoiceArea({ activity, selected, onSelect }: { activity: Activity; selected?: string; onSelect: (value: string) => void }) {
  return <View style={styles.choiceList}>{activity.choices?.map((choice, index) => <Pressable key={choice.id} onPress={() => onSelect(choice.id)} style={[styles.choice, selected === choice.id && styles.choiceSelected]}><View style={[styles.choiceIndex, selected === choice.id && styles.choiceIndexSelected]}><Text style={styles.choiceIndexText}>{index + 1}</Text></View><Text style={styles.choiceLabel}>{choice.label}</Text>{selected === choice.id && <MaterialCommunityIcons name="check-circle" size={22} color={Theme.colors.greenDark} />}</Pressable>)}</View>;
}
