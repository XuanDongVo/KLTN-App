...
export function AdminImageField({ value, onChange }: { value: BackendMedia; onChange: (value: BackendMedia) => void }) {
  ...
  return <View style={styles.root}>
    <View style={styles.heading}><Text style={styles.title}>Ảnh bìa</Text><Pressable disabled={uploading} onPress={() => void upload()} style={[styles.upload, uploading && styles.disabled]}>{uploading ? <ActivityIndicator color="#FFFFFF" /> : <MaterialCommunityIcons name="cloud-upload-outline" size={19} color="#FFFFFF" />}<Text style={styles.uploadText}>{uploading ? 'Đang tải' : 'Tải ảnh lên'}</Text></Pressable></View>
